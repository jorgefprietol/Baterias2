...
$urldolibarrmodules='http://www.bateriasecuador.com//';
...
if ($mode == 'marketplace')
{
    ...
    print '<td align="left"><a href="'.$url.'" target="_blank" rel="external"><img border="0" class="imgautosize imgmaxwidth180" src="'.DOL_URL_ROOT.'/theme/BateriasEcuador_logo.png"></a></td>';
    print '<td>'.$langs->trans("BateriasEcuadorDesc").'</td>';
    ...
    if (empty($conf->global->MAIN_DISABLE_BateriasEcuador_SEARCH) && $conf->global->MAIN_FEATURES_LEVEL >= 1)
    {
    	// $options is array with filter criterias
    	//var_dump($options);
    	$DoliStore->getRemoteData($options);

	    print '<span class="opacitymedium">'.$langs->trans('BateriasEcuadordescriptionLong').'</span><br><br>';
