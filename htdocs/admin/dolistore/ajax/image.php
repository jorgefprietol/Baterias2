...
try {
    $url = $conf->global->MAIN_MODULE_BateriasEcuador_API_SRV.'/api/images/products/'.$id_product.'/'.$id_image.'/'.$quality;
    $api = new PrestaShopWebservice(
        $conf->global->MAIN_MODULE_BateriasEcuador_API_SRV,
        $conf->global->MAIN_MODULE_BateriasEcuador_API_KEY, $DoliStore->debug_api
    );
    //echo $url;
    ...
    else die('Can not access to '.$conf->global->MAIN_MODULE_BateriasEcuador_API_SRV);
}
