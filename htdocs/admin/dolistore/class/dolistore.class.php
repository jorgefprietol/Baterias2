...
class DoliStore
{
	...
	function __construct($debug=false)
	{
		...
		$this->shop_url  = 'http://www.bateriasecuador.com//index.php?controller=product&id_product=';
	...
	function getRemoteData($options = array('start' => 0, 'end' => 10, 'per_page' => 50, 'categorie' => 0))
	{
		...
		try {
			$this->api = new PrestaShopWebservice($conf->global->MAIN_MODULE_BateriasEcuador_API_SRV, $conf->global->MAIN_MODULE_BateriasEcuador_API_KEY, $this->debug_api);
			dol_syslog("Call API with MAIN_MODULE_BateriasEcuador_API_SRV = ".$conf->global->MAIN_MODULE_BateriasEcuador_API_SRV);
			// $conf->global->MAIN_MODULE_BateriasEcuador_API_KEY is for the login of basic auth. There is no password as it is public data.
			...
			if ($this->search != '') {
				$opt2['url'] = $conf->global->MAIN_MODULE_BateriasEcuador_API_SRV.'/api/search?query='.$this->search.'&language='.$this->lang;
			...
			{
				print 'Can not access to '.$conf->global->MAIN_MODULE_BateriasEcuador_API_SRV.'<br>';
				print $e->getMessage();
			}
		}
	}
